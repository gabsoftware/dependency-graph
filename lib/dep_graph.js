...
DepGraph.prototype = {
  ...
  "steps": function () {
    ...
    var processchild = function( parent, lvl ) {
      ...
      pass = true;
      for( cpt = 0, nbr = graph.incomingEdges[ parent ].length; cpt < nbr; cpt++ ) {
        check = graph.incomingEdges[ parent ][ cpt ];
      ...
      if( pass ) {
        ...
        for( cpt = 0, nbr = graph.outgoingEdges[ parent ].length; cpt < nbr; cpt++ ) {
          child = graph.outgoingEdges[ parent ][ cpt ];
    ...
    if( steps.length ) {
      ...
      for( var property in graph.incomingEdges ) {
        if( graph.incomingEdges.hasOwnProperty( property ) ) {

          // get the array of child dependencies for this entry
          value = graph.incomingEdges[ property ];
